...
function SocialTool({ app }) {
  ...
  const [data, setData] = useState(null)
    ...
    const onSubmit = async (e) => {
      ...
      try {
        const data = await SOCIAL(JSON.stringify(x))
        console.log(data);
        setData(data)
      } catch (error) {
  ...
  return (
    <form onSubmit={onSubmit} className='flex flex-col justify-start items-start w-full gap-4 relative'>
        <div className='animate opacity-0 w-full'>
          <Input label='Topic' required={true} type='text' placeholder='Enter Topic Here' readOnly={false} onChange={(e) => setTopic(e.target.value)}/>
        </div>
        <span className='flex flex-col gap-2 w-full animate opacity-0 relative z-50'>
          <label className='font-Satoshi font-normal text-base text-black-50'>Select Platform</label>
          <Select defaultValue={platform} styles={window.innerWidth< 768? selectStylesMob: selectStyles} options={platforms} onChange={(o)=>setPlatform(o)} components={{ IndicatorSeparator: () => null }}/>
        </span>
          ...
          <div className='px-5 border-[1px] border-grey rounded-lg py-2'>
              {files.map((f, index) => (
                <>
                  ...
                  <hr className='w-[40%] h-[2px] bg-black-50 opacity-30 mt-1 mb-2 sm:w-full sm:mb-0' />
              ...
              <div className='flex justify-between items-center sm:flex-col sm:gap-4'>
                <div className='flex justify-start items-center gap-3' {...getRootProps()}>
                  ...
                  <svg width="52" height="52" viewBox="0 0 58 58" fill="none" xmlns="http://www.w3.org/2000/svg" className='sm:w-[40px] sm:h-[40px]'>
                  ...
                  <div>
                    ...
                    <p className='text-black-30 font-Satoshi leading-5 font-medium text-sm sm:text-[12px]'>Limit 200MB per file • PDF, DOCX</p>
                  </div>
                </div>
        ...
        <button disabled={loading} className='disabled:opacity-70 disabled:cursor-wait hover:opacity-70 my-3 rounded-md opacity-0 animate px-5 py-1.5 font-Satoshi font-medium text-sm pb-2 bg-green-200 text-white cursor-pointer transition-all ease-in duration-150 border-[1px] border-grey'>
          Generate Pocial Media Posts
        </button>

       {data &&
        <div className='flex justify-start items-start gap-4'>
          <span className=' w-[35px] h-[35px] rounded-md bg-[#F7D4E2] p-[1px]'>
            <img src={app.img} className='w-full h-full object-cover' alt={app.name}/>
          </span>
          <TypingEffect text={data}/>
        </div>   }
    </form>
  )
}
